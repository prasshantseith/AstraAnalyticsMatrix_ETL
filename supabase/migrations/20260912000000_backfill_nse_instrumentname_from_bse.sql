...
DECLARE
    v_row_count INT;
BEGIN
    -- NSE bhavcopy sends the ticker symbol as the instrument name instead of
    -- the full company name. Backfill those rows from the matching BSE
    -- listing (same ISIN) before computing performance, so NSE-only viewers
    -- don't see a raw ticker where a name is expected.
    UPDATE "Stocks"."StockData" AS nse
    SET "InstrumentName" = bse."InstrumentName"
    FROM (
        SELECT DISTINCT ON ("IsinCode")
            "IsinCode",
            "InstrumentName"
        FROM "Stocks"."StockData"
        WHERE "StockExchangeCode" = 'BSE'
        ORDER BY "IsinCode", "InstrumentName"   -- deterministic pick when an ISIN has >1 BSE name
    ) AS bse
    WHERE nse."StockExchangeCode" = 'NSE'
      AND nse."TickerSymbol" = nse."InstrumentName"
      AND nse."IsinCode" = bse."IsinCode";
